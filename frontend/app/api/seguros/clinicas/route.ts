...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!seguroId) {
      return NextResponse.json(
        { error: 'seguroId is required' },
        { status: 400 }
      );
    }

    const params = new URLSearchParams({ seguroId });

    const url = `${AWS_BASE_URL}/seguros-clinicas?${params.toString()}`;
    ...
    console.error('Seguros clinicas error:', error);
    return NextResponse.json(
