...
def remove_columns(input_file, output_file, columns_to_remove, input_delimiter=';', output_delimiter=','):
    """
    Remove specified columns from a CSV file and optionally change delimiter.
    ...
    Args:
        ...
        output_delimiter: Output CSV delimiter (default: ',')
    """
    ...
    with open(input_path, 'r', encoding='utf-8') as infile:
        ...
        with open(temp_file, 'w', encoding='utf-8', newline='') as outfile:
            writer = csv.DictWriter(outfile, fieldnames=columns_to_keep, delimiter=output_delimiter)
            writer.writeheader()
    ...
    print(f"Delimiter changed from '{input_delimiter}' to '{output_delimiter}'")
    return True
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  # Remove columns from internacional_clean.csv (in-place)
  python clean-up.py internacional_clean.csv --remove addiid documentId fullname slug profile_url
  
  # Remove columns and save to new file
  python clean-up.py input.csv output.csv --remove column1 column2
  
  # Remove columns interactively
  python clean-up.py internacional_clean.csv --remove addiid documentId --interactive
        """
    )
    ...
    parser.add_argument('--input-delimiter', '-id', default=';',
                       help='Input CSV delimiter (default: ;)')
    parser.add_argument('--output-delimiter', '-od', default=',',
    ...
    print(f"Input delimiter: '{args.input_delimiter}'")
    print(f"Output delimiter: '{args.output_delimiter}'")
    ...
    # Process the file
    success = remove_columns(args.input_file, output_file, args.remove, args.input_delimiter, args.output_delimiter)
